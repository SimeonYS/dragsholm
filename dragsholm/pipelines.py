...
class DragsholmPipeline:

    # Database setup
    conn = sqlite3.connect('dragsholm.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dragsholm`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dragsholm WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dragsholm (date, title, link, content)"
